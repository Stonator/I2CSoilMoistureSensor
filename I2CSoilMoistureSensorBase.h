...
#ifndef I2CSOILMOISTURESENSORBASE_H
#define I2CSOILMOISTURESENSORBASE_H
...
class I2CSoilMoistureSensorBase
{
public:
  // Optionally set sensor I2C address if different from default
  I2CSoilMoistureSensorBase(uint8_t address = SOILMOISTURESENSOR_DEFAULT_ADDR);
...
protected:
  ...
  virtual bool writeI2CRegister8bit(uint8_t address, uint8_t value) = 0;

  // Helper method to write an 8 bit value to the sensor via I2C to the given register
  virtual bool writeI2CRegister8bit(uint8_t address, uint8_t reg, uint8_t value) = 0;

  // Helper method to read a 8 bit value from the given register
  virtual uint8_t readI2CRegister8bit(uint8_t address, uint8_t reg) = 0;

  // Helper method to read a 16 bit unsigned value from the given register
  virtual uint16_t readI2CRegister16bitUnsigned(uint8_t address, uint8_t reg) = 0;

  // Helper method to read a 16 bit signed value from the given register
  int16_t readI2CRegister16bitSigned(uint8_t address, uint8_t reg);
